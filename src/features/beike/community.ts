...
const getCommunity = async () => {
  ...
  for (let q = 0; q < communityList.length; q++) {
    const item = communityList[q];
    const [err, res] = await to(axios.get(`https://cq.ke.com/ershoufang/co32f2l3c${item.id}`))
    if (err) {
      log('小区数据抓取失败~')
      log(err)
    }
    ...
    const [err2, res2] = await to(axios.get(`https://cq.ke.com/api/listtop?type=resblock&resblock_id=${item.id}&community_id=0&district_id=&bizcircle_id=&subway_station_id=&word=&source=ershou_xiaoqu`))
    if (err2) {
      log('小区数据2抓取失败~')
      log(err2)
    }
    ...
    const [err3, res3] = await to(connection.query(`insert into community set ?`, data))
    if (err3) {
      log('插入数据失败~')
      console.log(err3)
      return
